"""
Receiver (RX) digital baseband (DBB) definitions.
...
class RxDbbScFde(RxDbb):
    """SC-FDE RX DBB, as presented in MDPI SI paper.
    """
...
class RxDbbScFdeInitial(RxDbb):
    """SC-FDE RX DBB initial iteration, without joint estimation and equalization blocks
    """
...
class RxDbbScFdeSimplified():
    """Simplified SC-FDE RX DBB:
        - Blocks include hardcoded properties, assuming they know which blocs succceed them (no data-compensatory block)
        - Blocks not contributing to latency are removed (event reduction)
    """
    ...
    def __init__(self, log_path, log_subdir, env, MCS, num_of_blocks, block_padding, decoder_iterations, demapper_delay_instance):
        ...
        frequency_domain_channel_equalization = Block.FDE_Simple(
            block_id=1,
            env=env,
            log_path=log_path,
            log_subdir=log_subdir,
        ...
        demapping_block = Block.Demapper_Simple(
            block_id=2,
            env=env,
            log_path=log_path,
            log_subdir=log_subdir,
...
class RxDbbScFdeSimplified448():
    """Simplified SC-FDE RX DBB:
        - Blocks include hardcoded properties, assuming they know which blocs succceed them (no data-compensatory block)
        - Blocks not contributing to latency are removed (event reduction)
        - Demapper operates on entire 448-long sequences (event reduction)
    """
